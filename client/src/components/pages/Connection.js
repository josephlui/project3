...
class Connection extends React.Component {
  ...
  onSelect = clientId => {
    ...
    const client = this.state.userList.filter(user => user.userId === clientId)[0];
      ...
      .then(result => {
        if (result.status === 200) {
          ...
          return {
            ...
            userList: state.userList.filter (user => user.userId !== client.userId)
          }
        });
        } else {
  ...
  render() {
    ...
    return <div>Friend's connection page
    <br/>
    {this.state.userId}
    <br/>
    {this.state.name}
